...
```{r 1_load}
peso_rata <- read.xlsx("anova II.xlsx", sheetIndex = 1, encoding = "UTF-8")
...
pander(ddply(peso_rata, .(Fuente, Proteina), summarise, 
             ...
             sd = sd(Ganancia_Peso)))
pander(table("sumario_1", "Medias, varianzas y desvíos  estándar por grupo"))
       
pander(bartlett.test(Ganancia_Peso ~ interaction(Fuente, Proteina), 
                     data = peso_rata))
pander(table("btest_1", "Test de Bartlett"))
...
```{r 1_anova_plot}
autoplot(peso_rata_aov, which = c(1:3, 5), mfrow = c(2,2))
```
...
```{r 2_load}
huevos <- read.xlsx("anova II.xlsx", sheetIndex = 2, encoding = "UTF-8")
...
autoplot(huevos_aov, which = c(1:3,5 ), mfrow = c(2,2))
```
...
```{r 3_load}
sangrado <- read.xlsx("anova II.xlsx", sheetIndex = 3, encoding = "UTF-8")
pander(sangrado)
```
...
autoplot(sangrado_aov, which = c(1:3, 5), mfrow = c(2, 2))
```
...
```{r 4_load}
yogur <- read.xlsx("anova II.xlsx", sheetIndex = 4, encoding = "UTF-8", 
                   startRow = 2)
pander(yogur)
```
...
autoplot(yogur_aov, which = c(1:3, 5), mfrow = c(2, 2))
...
```{r 5_load}
cerdos <- read.xlsx("anova II.xlsx", sheetIndex = 5, encoding = "UTF-8")
pander(cerdos)
```
...
autoplot(cerdos_aov, which = c(1:2, 5), mfrow = c(2, 2))
```
...
```{r 6_load}
levadura <- read.xlsx("anova II.xlsx", sheetIndex = 6, encoding = "UTF-8")
...
autoplot(levadura_aov, which = c(1:3, 5), mfrow = c(2,2))
```
...
```{r 7_load}
lago <- read.xlsx("anova II.xlsx", sheetIndex = 7, encoding = "UTF-8", startRow = 2)
pander(lago)
...
autoplot(lago_aov, which = c(1:3, 5), mfrow = c(2,2))
```
